...
class CustomChoices extends Component
{
    public string $uuid;
    ...
    public function __construct(
        ...
        public ?string $tooltipMessage = 'Select an option',
        public ?string $onCreateNew = null,
                ...
                <div x-data="{ focused: false, selection: @entangle($attributes->wire('model')) }">
                    ...
                    >
                        ...
                        >
                            <!-- ICON  -->
                            @if($icon)
                                <x-mary-icon :name="$icon" class="absolute top-1/2 -translate-y-1/2 start-3 text-gray-400 pointer-events-none text-md" />
                            @endif

                            <!-- CLEAR ICON  -->
                            @if(! $isReadonly() && ! $isDisabled())
                                <x-mary-icon @click="reset()"  name="o-x-mark" x-show="!isSelectionEmpty" class="absolute top-1/2 end-2 -translate-y-1/2 cursor-pointer text-gray-400 hover:text-gray-600" dusk="clear-choices"/>
                            @endif
                            ...
                            <div wire:key="options-list-{{ $uuid }}" class="{{ $height }} w-full absolute z-10 shadow-xl bg-base-100 border border-base-300 rounded-lg cursor-pointer overflow-y-auto" x-anchor.bottom-start="$refs.container">
                               ...
                               @endif
                                ...
                                <div
                                    ...
                                    class="p-3 decoration-wavy decoration-warning underline font-bold border border-s-4 border-s-warning border-b-base-200 flex flex-col gap-2"
                                >
                                    <p>
                                        <span class="block max-w-sm truncate ..." x-text="`${$refs.searchInput.value}`"></span>
